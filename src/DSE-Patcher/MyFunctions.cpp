...
int MyInitVulnerableDrivers(VULNERABLE_DRIVER *vd,DWORD dwElements)
{
	...
	// DBUtil v2.7 (index 0)
	{
		VULNERABLE_DRIVER *v = &vd[0];
		v->szProvider = "DBUtil v2.7 (from Dell OptiPlex 7070 v1.10.0 BIOS Update)";
		v->szToolTipText = "DBUtil v2.7 supports Windows 8 and later.\n\nAttention:\nTo install the driver on Windows 7 with Service Pack 1\nyou have to apply the Windows update KB3033929\nfor SHA256 support.";
		v->pFunctionOpenDevice = MyDBUtilOpenDevice;
		v->pFunctionReadMemory = MyDBUtilReadMemory;
		v->pFunctionWriteMemory = MyDBUtilWriteMemory;
		v->pFunctionStartDriver = MyInstallDriver;
		v->pFunctionStopDriver = MyUninstallDriver;
		v->szServiceName = "DBUtilDrv2";
		// Attention: DBUtil v2.7 has the device name "\\.\DBUtil_2_5". This is the same device name as in version 2.5!
		v->szDriverSymLink = "\\\\.\\DBUtil_2_5";

		// files: sys, inf, cat, WdfCoInstaller
		lstrcpy(v->driverFile[0].szFilePath,szPath);
		lstrcat(v->driverFile[0].szFilePath,"\\DBUtilDrv2.sys");
		v->driverFile[0].bData = DBUtil_v27_sys;
		v->driverFile[0].dwSize = sizeof(DBUtil_v27_sys);

		lstrcpy(v->driverFile[1].szFilePath,szPath);
		lstrcat(v->driverFile[1].szFilePath,"\\dbutildrv2.inf");
		v->driverFile[1].bData = DBUtil_v27_inf;
		v->driverFile[1].dwSize = sizeof(DBUtil_v27_inf);

		lstrcpy(v->driverFile[2].szFilePath,szPath);
		lstrcat(v->driverFile[2].szFilePath,"\\DBUtilDrv2.cat");
		v->driverFile[2].bData = DBUtil_v27_cat;
		v->driverFile[2].dwSize = sizeof(DBUtil_v27_cat);

		lstrcpy(v->driverFile[3].szFilePath,szPath);
		lstrcat(v->driverFile[3].szFilePath,"\\WdfCoInstaller01009.dll");
		v->driverFile[3].bData = DBUtil_v27_WdfCI;
		v->driverFile[3].dwSize = sizeof(DBUtil_v27_WdfCI);

		v->szHardwareId = "ROOT\\DBUtilDrv2";
		v->dwMinSupportedBuildNumber = 9200;
		v->dwMaxSupportedBuildNumber = 0xFFFFFFFF;
	}

	// DBUtil v2.6 (index 1)
	{
		VULNERABLE_DRIVER *v = &vd[1];
		v->szProvider = "DBUtil v2.6 (from Dell OptiPlex 7070 v1.7.2 BIOS Update)";
		v->szToolTipText = "DBUtil v2.6 supports Windows 10 version 1507 and later.\n\nAttention:\nThe driver does not install on older OSs, because the\ndependent KMDF library version 1.15 is not present.";
		v->pFunctionOpenDevice = MyDBUtilOpenDevice;
		v->pFunctionReadMemory = MyDBUtilReadMemory;
		v->pFunctionWriteMemory = MyDBUtilWriteMemory;
		v->pFunctionStartDriver = MyInstallDriver;
		...
		lstrcpy(v->driverFile[0].szFilePath,szPath);
		lstrcat(v->driverFile[0].szFilePath,"\\DBUtilDrv2.sys");
		v->driverFile[0].bData = DBUtil_v26_sys;
		v->driverFile[0].dwSize = sizeof(DBUtil_v26_sys);

		lstrcpy(v->driverFile[1].szFilePath,szPath);
		lstrcat(v->driverFile[1].szFilePath,"\\dbutildrv2.inf");
		v->driverFile[1].bData = DBUtil_v26_inf;
		v->driverFile[1].dwSize = sizeof(DBUtil_v26_inf);

		lstrcpy(v->driverFile[2].szFilePath,szPath);
		lstrcat(v->driverFile[2].szFilePath,"\\DBUtilDrv2.cat");
		v->driverFile[2].bData = DBUtil_v26_cat;
		v->driverFile[2].dwSize = sizeof(DBUtil_v26_cat);

		v->szHardwareId = "ROOT\\DBUtilDrv2";
		v->dwMinSupportedBuildNumber = 10240;
		v->dwMaxSupportedBuildNumber = 0xFFFFFFFF;
	}

	// DBUtil v2.5 (index 2)
	{
		VULNERABLE_DRIVER *v = &vd[2];
		v->szProvider = "DBUtil v2.5 (from Dell OptiPlex 7070 v1.7.0 BIOS Update)";
		v->szToolTipText = "DBUtil v2.5 supports Windows 10 version 1507 and later.\n\nAttention:\nThe driver does not install on older OSs, because the\ndependent KMDF library version 1.15 is not present.";
		v->pFunctionOpenDevice = MyDBUtilOpenDevice;
		v->pFunctionReadMemory = MyDBUtilReadMemory;
		v->pFunctionWriteMemory = MyDBUtilWriteMemory;
		v->pFunctionStartDriver = MyInstallDriver;
		...
		lstrcpy(v->driverFile[0].szFilePath,szPath);
		lstrcat(v->driverFile[0].szFilePath,"\\DBUtilDrv2.sys");
		v->driverFile[0].bData = DBUtil_v25_sys;
		v->driverFile[0].dwSize = sizeof(DBUtil_v25_sys);

		lstrcpy(v->driverFile[1].szFilePath,szPath);
		lstrcat(v->driverFile[1].szFilePath,"\\dbutildrv2.inf");
		v->driverFile[1].bData = DBUtil_v25_inf;
		v->driverFile[1].dwSize = sizeof(DBUtil_v25_inf);

		lstrcpy(v->driverFile[2].szFilePath,szPath);
		lstrcat(v->driverFile[2].szFilePath,"\\DBUtilDrv2.cat");
		v->driverFile[2].bData = DBUtil_v25_cat;
		v->driverFile[2].dwSize = sizeof(DBUtil_v25_cat);
	...
	// DBUtil v2.3 (index 3)
	{
		VULNERABLE_DRIVER *v = &vd[3];
		v->szProvider = "DBUtil v2.3 (from Dell OptiPlex 7070 v1.0.2 BIOS Update)";
		v->szToolTipText = "DBUtil v2.3 supports Windows Vista and later up to\nWindows 11 Build 21H2.\n\nAttention:\nThe driver does not support Windows 11 Build 22H2,\nbecause the driver blocklist is updated by Microsoft.\n\nThe driver can only be manually deleted on Windows 10\nafter a reboot, because it is not stoppable and running.\nOn Windows 7 the deletion works without any problems.";
		v->pFunctionOpenDevice = MyDBUtilOpenDevice;
		v->pFunctionReadMemory = MyDBUtilReadMemory;
		v->pFunctionWriteMemory = MyDBUtilWriteMemory;
		v->pFunctionStartDriver = MyCreateAndStartService;
		v->pFunctionStopDriver = MyStopAndDeleteService;
		v->szServiceName = "DBUtil_2_3";
		v->szDriverSymLink = "\\\\.\\DBUtil_2_3";

		// files: sys only
		lstrcpy(v->driverFile[0].szFilePath,szPath);
		lstrcat(v->driverFile[0].szFilePath,"\\DBUtil_2_3.Sys");
		v->driverFile[0].bData = DBUtil_v23_sys;
		v->driverFile[0].dwSize = sizeof(DBUtil_v23_sys);

		v->dwMinSupportedBuildNumber = 6000;
		v->dwMaxSupportedBuildNumber = 22000;
	}
